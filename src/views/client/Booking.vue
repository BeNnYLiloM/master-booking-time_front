...
const getWorkingDaysSummary = () => {
  ...
  if (dates.length === 0) return 'Не указано';
  
  // Берем первые 3 ближайших рабочих дня
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  ...
  if (upcomingDates.length === 0) return 'Нет доступных дней';
  ...
  return dates.length > 3 ? `${summary} и еще...` : summary;
};
</script>
...
<template>
  <div class="min-h-screen min-h-dvh pb-28 animate-fade-in">
    <!-- Master Info Card -->
    <div v-if="masterProfile" class="bg-gradient-to-br from-accent/10 to-gradient-end/10 p-4">
      <div class="card">
        <div class="flex items-start gap-3 mb-4">
          <!-- Master Avatar -->
          <div 
            v-if="masterProfile.avatarUrl"
        ...
        <!-- Location Info -->
        <div v-if="masterProfile.location?.address?.text" class="flex items-start gap-2 mb-3 p-3 rounded-lg bg-tg-bg">
          ...
          <div class="flex-1 min-w-0">
            <div class="text-xs text-tg-hint mb-1">Адрес</div>
            <div class="text-sm">{{ masterProfile.location.address.text }}</div>
          </div>
        </div>

        <!-- Phone Number -->
        <div v-if="masterProfile.phoneNumber" class="flex items-start gap-2 mb-3 p-3 rounded-lg bg-tg-bg">
          ...
          <div class="flex-1 min-w-0">
            <div class="text-xs text-tg-hint mb-1">Телефон</div>
            <a :href="'tel:' + masterProfile.phoneNumber" class="text-sm text-accent hover:underline">
              {{ masterProfile.phoneNumber }}
            </a>
          </div>
        </div>

        <!-- Working Schedule Summary -->
        <div v-if="Object.keys(masterWorkingDates).length > 0" class="flex items-start gap-2 p-3 rounded-lg bg-tg-bg">
          <svg class="w-5 h-5 text-green-500 shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
          </svg>
          <div class="flex-1">
            <div class="text-xs text-tg-hint mb-1">График работы</div>
            <div class="text-sm">{{ getWorkingDaysSummary() }}</div>
          </div>
        </div>
      </div>
    </div>
